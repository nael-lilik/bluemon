...
def log_devices(devices, previous_devices, memory):
...
def log_status(previous_devices):
    with open('bluetooth_log.txt', 'a') as file:
        for addr, data in previous_devices.items():
            ...
            file.write(f"Device {addr} appeared at {appeared}, Stay duration: {stay_duration} seconds")
            if disappeared:
...
while True:
    try:
        ...
        previous_devices, memory = log_devices(devices, previous_devices, memory)
